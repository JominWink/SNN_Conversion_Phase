...
from CIFAR.models.spiking_layer import SpikeModule, SpikeModel, lp_loss, IFNeuron, mem_update
...
def bias_corr_model(model: SpikeModel, train_loader: torch.utils.data.DataLoader, correct_mempot: bool = False,
    ...
    for (input, target) in train_loader:
        ...
        for name, module in model.named_modules():
            if isinstance(module, (SpikeResModule_CIFAR, SpikeModule)):
...
def emp_bias_corr(model: SpikeModel, module: SpikeModule, train_data: torch.Tensor, correct_mempot: bool = False,
                  learning_rate: float = 1e-5, optimize_iter: int = 5000, loss_func = lp_loss):
    ...
    snn_out = get_out(train_data)[1] / model.T
...
class ActivationSaverHook:
    ...
    def __call__(self, module, input_batch, output_batch):
        ...
        if self.stored_input is None:
            ...
            self.input.append(input_batch[0])
        else:
...
class GetLayerInputOutput:
    ...
    def __call__(self, input):
        ...
        return self.data_saver.stored_input.detach(), self.data_saver.stored_output.detach(), \
               self.data_saver.stored_residual, self.data_saver.input
        # return self.data_saver.stored_input.detach(), self.data_saver.stored_output.detach(), \
...
def weights_cali_model(model: SpikeModel, train_loader: torch.utils.data.DataLoader,
                       ...
                       batch_size: int = 64, num_cali_samples: int = 1024, dist_avg: bool = False):
    """
...
def weights_cali_res_layer(model: SpikeModel, module: SpikeModule, data_sample: torch.Tensor,
    ...
    for i in range(int(data_sample.size(0) / batch_size)):
        ...
        model.set_spike_state(use_spike=False)
        _, cur_out, _, _ = get_out(data_sample[i * batch_size:(i + 1) * batch_size])
        get_out.data_saver.reset()
        # compute the spike input
        model.set_spike_state(use_spike=True)
        cur_inp, _, cur_res, _ = get_out(data_sample[i * batch_size:(i + 1) * batch_size])
        get_out.data_saver.reset()
    ...
    for i in range(optimize_iter):
        ...
        IF = IFNeuron(module.threshold, module.mem_pot_init, module.sim_length, module.fwd_func, module.weight,
                      module.bias, module.fwd_kwargs)
        snn_out = IF(cur_inp, cur_res)
...
def weights_cali_layer(model: SpikeModel, module: SpikeModule, data_sample: torch.Tensor,
                       learning_rate: float = 1e-5, optimize_iter: int = 10000,
                       batch_size: int = 64, num_cali_samples: int = 1024, keep_gpu: bool = True,
                       loss_func=lp_loss, dist_avg: bool = False, index: int = 0):

    get_out = GetLayerInputOutput(model, module)
    device = next(module.parameters()).device
    data_sample = data_sample.to(device)
    cached_batches = []
    index = index * module.sim_length
    ...
    for i in range(int(data_sample.size(0) / batch_size)):
        ...
        # compute the spike input
        model.set_spike_state(use_spike=True)
        cur_inp, _, _, inp = get_out(data_sample[i * batch_size:(i + 1) * batch_size])
        get_out.data_saver.reset()
        cached_batches.append((cur_inp.cpu(), cur_out.cpu(), inp))

    # print(len(inp), inp[0].shape, len(cached_batches))
    ...
    input = []
    for t in range(module.sim_length):
        input.append(torch.cat([x[2][t] for x in cached_batches]))
    # print(cached_inps.shape, cached_outs.shape, len(input), input[0].shape, input[1].shape)
    # print(loss_func((input[0][5] + input[1][5]).cpu(), cached_inps[5].cpu()))
    ...
    for i in range(optimize_iter):
        idx = torch.randperm(num_cali_samples)[:batch_size]
        cur = cached_inps[idx].to(device)
        cur_inp = []
        for t in range(model.T):
            cur_inp.append(input[t][idx].to(device))
        cur_out = cached_outs[idx].to(device)
        # print(len(cur_inp), cur_inp[0].shape, cur_out.shape, loss_func(cur_inp[0][5] + cur_inp[1][5], cur[5]))

        optimizer.zero_grad()
        module.zero_grad()

        IF = IFNeuron(module.threshold, module.mem_pot_init, module.sim_length, module.fwd_func, module.weight, module.bias, module.fwd_kwargs)
        snn_out = IF(cur_inp)
