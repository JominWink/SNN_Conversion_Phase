...
def fold_bn_into_conv(conv_module, bn_module, avg=False): #conv_module = None
...
def search_fold_and_remove_bn(model):
    model.eval() #为了排除Dropout和BN对测试的影响
    prev = None
    for n, m in model.named_children():
        if is_bn(m) and is_absorbing(prev):
            fold_bn_into_conv(prev, m) #将bn转换成直连
            # set the bn module to straight through
            setattr(model, n, StraightThrough()) #它最基础的功能是修改类实例对象中的属性值
        elif is_absorbing(m):
            prev = m
        else:
